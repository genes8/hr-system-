...
// Dodajte ovu definiciju tipa
type PerformanceEntry = {
  name: string;
  performance: number;
};

// Ažurirajte employees niz
const employees: PerformanceEntry[] = [
  { name: "Ana Petrović", performance: 4.5 },
  { name: "Marko Jovanović", performance: 3.8 },
  { name: "Jelena Nikolić", performance: 4.2 },
  { name: "Stefan Đorđević", performance: 3.5 },
];

// Ažurirajte customTooltip funkciju
    ...
    return (
      <div className="bg-white p-4 rounded-lg shadow-md border border-gray-200">
        ...
        <p className="text-sm text-gray-600">
          Performanse: <span className="font-semibold text-blue-600">{payload[0].value}</span>
        </p>
      </div>
    );
  }
...
// Ažurirajte getBarColor funkciju
...
export default function UpravljanjeTalentima() {
                    ...
                    <CardContent>
                      <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={employees}>
                          ...
                          <Legend />
                          <Bar dataKey="performance" name="Performanse">
                            {employees.map((entry, index) => (
                              <Cell key={`cell-${index}`} fill={getBarColor(Number(entry.performance))} />
                            ))}
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
